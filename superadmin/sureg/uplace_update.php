...
{
	//$admin_id=$_GET["admin_id"];
	$data = mysql_query("SELECT * FROM uplace_update ORDER BY id DESC") or die("Error in Selection Query <br> ".$query."<br>". mysql_error());
	...
	if($rowcount>0){
		...
		?>
				...
				<script>
				$('#notify_count_uplace_update').html('<?php echo $rowcount;?>');
				</script>
				
				<li class="list_data_uplace_update" onclick="list_data_get_uplace_update('<?php echo $row['id'];?>');">
				  <a>
		...
		?>
			<script>
			$('#notify_count_uplace_update').html('0');
			</script>
...
if(isset($_GET["load_data"]))
{
	...
	$data = mysql_query("SELECT * FROM uplace_update WHERE id='$notify_id'") or die("Error in Selection Query <br> ".$query."<br>". mysql_error());
		while($row=mysql_fetch_array($data)) 
		{
?>
				...
				<br><a href="javascript:void(0);" onclick="delete_notify_uplace_update(<?php echo $row['id']; ?>);">
...
if(isset($_GET["delete_notify"]))
{
	...
	// $admin_id = $_GET["admin_id"];
	 ...
	 $del ="DELETE FROM uplace_update WHERE id='$notify_id'";
